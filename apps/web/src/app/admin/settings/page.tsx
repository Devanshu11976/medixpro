...
import { User, Shield, Bell, CheckCircle, Save } from "lucide-react";
...
export default function AdminSettingsPage() {
  ...
  const [expiryAlert, setExpiryAlert] = useState("60");
        ...
        <div className="space-y-6 px-6 py-6 lg:px-8">
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* ERP Business Preferences (Admin only) */}
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <div className="flex items-center gap-3 border-b border-gray-100 pb-4 mb-5">
                  ...
                  <div>
                    <h3 className="font-display font-semibold text-gray-900">Inventory Alarms & Thresholds</h3>
                    <p className="text-xs text-gray-500">Tune operational telemetry triggers.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="expiryalert">Expiry Warning Period (days)</Label>
                    <Input 
                      id="expiryalert" 
                      type="number" 
                      value={expiryAlert}
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm text-center">
                ...
                <h4 className="font-display font-bold text-gray-900 text-base">{name || "Admin User"}</h4>
                <span className="inline-block mt-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-wider">
                  {user?.role || "Admin"}
                </span>
                ...
                <div className="mt-5 border-t border-gray-100 pt-4 text-left space-y-2">
                  <div>
                    ...
                    <span className="text-xs text-gray-700 font-semibold">{email || "admin@medixpro.com"}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-semibold text-gray-400 block uppercase">Permissions Status</span>
                    <span className="text-xs text-green-600 font-bold">Active Wholesaler</span>
                  </div>
                </div>
              </div>
